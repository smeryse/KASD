...
internal static class LongestCommonSubstring
{
    static void Main()
    {
        ...
        int separator = firstLength;

        int[] suffixArray = SuffixArrayTask.BuildSuffixArray(joined);
        int[] lcp = SuffixArrayTask.BuildLcp(joined, suffixArray);
        ...
        {
            int a = suffixArray[i];
            int b = suffixArray[i + 1];
            bool aInFirst = a < separator;
            bool bInFirst = b < separator;

            if (a == separator || b == separator || aInFirst == bInFirst) continue;
            if (lcp[i] <= bestLen) continue;

            bestLen = lcp[i];
            bestPos = aInFirst ? a : b;
        }

        Console.WriteLine(bestLen == 0 ? "" : joined.Substring(bestPos, bestLen));
    }
