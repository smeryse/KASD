...
internal static class SubstringCount
{
    public static void Main()
    {
        ...
        int[] suffixArray = SuffixArrayTask.BuildSuffixArray(s);
        int[] lcp = SuffixArrayTask.BuildLcp(s, suffixArray);

        long n = s.Length;
        long answer = n * (n + 1) / 2;
        for (int i = 0; i < lcp.Length; i++) answer -= lcp[i];
