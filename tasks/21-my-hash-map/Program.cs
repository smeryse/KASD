...
namespace Task21
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var map4 = new MyHashMap<string, int>(4, 0.75f); 
            for (int i = 0; i < 20; i++)
            {
                map4.Put($"key{i}", i);
            }
            ...
            var map5 = new MyHashMap<string, int>(); 
            Show($"Default constructor: capacity=16, loadFactor=0.75");
            var map6 = new MyHashMap<string, int>(32); 
            Show($"Custom capacity: 32");
            var map7 = new MyHashMap<string, int>(16, 0.5f); 
